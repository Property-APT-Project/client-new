...
const navigateToAptMap = () => {
  fetchApartments().then((response) => {
    apartments.value = response[0];
    router.push({
      name: "aptKeyword",
      params: { keyword: apartments.value },
    });
  });
};
...
<template>
  <div>
    ...
    <div
      v-if="!isRequested"
      class="col-lg-12 mt-5 mb-5 justify-content-center row"
    >
      <div
        class="card align-items-center justify-content-center rounded-pill col-9"
      >
        <div
          class="p-3 card-body align-items-center justify-content-center w-100"
        >
          <div class="d-flex align-items-center gap-6 w-100 p-0">
            <!-- <form class="w-100"> -->

            <div class="row w-100 m-1 d-flex align-items-center">
